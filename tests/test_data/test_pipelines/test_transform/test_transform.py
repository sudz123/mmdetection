...
def test_simple_copy_paste():
    ...
    assert np.array([1., 269., 512., 281.]).all() == flipped_bboxes[0].all()
    assert np.array([9.,  56., 508.,  84.]).all() == flipped_bboxes[1].all()
    ...
    # test assertion for box occlusion and box size check
